...
from FUNCTION.generalFunctions import fetch_combined_user_details, function_fetch_all_applicant_results
...
def AdminDecisionMaking():
    ...
    return render_template('AdministratorDecision.html',  distinct_data=distinct_data,
                            recent_registrations=recent_registrations,
                            incomplete_profiles=total_incomplete_profiles,
                            cv_data_admin_view_all_cv=cv_data
                            )
...
def Making_approval_or_refual_decision_on_learner():
    try:
        ...
        print("Email collected from frontend:", email)
        print("Decision outcome selected:", decision_outcome)
        print("Reason for rejection:", reason_for_rejection)
        ...
        cursor.execute("""
            INSERT INTO SSDD_COMPLETE_STUDENT_DECISION_OUTCOME (email, Decision_outcome, Administrator_name, Decision_date)
            VALUES (?, ?, ?, ?)
        """, (email, decision_outcome, administrator_email, decision_date))
        print("Inserted decision into the database successfully!")

        # If decision is "Approve", update Is_Active in SSDD_COMPLETE_LOGIN
        if decision_outcome == "Approve":
            print("Updating Is_Active in SSDD_COMPLETE_LOGIN for", email)
            cursor.execute("""
                UPDATE SSDD_COMPLETE_LOGIN
                SET Is_Active = 1
                WHERE email = ?
