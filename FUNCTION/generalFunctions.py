...
# Fetch all applicant results by the administrator
...
# Fetching results for the company
def function_fetch_all_applicant_results_for_company():
    ...
    try:
        ...
        # ✅ Fixed Query (Ensuring Province is handled correctly)
        query = """
            SELECT 
                p.firstname + ' ' + p.lastname AS name,
                COALESCE(MAX(e.Field_of_study), 'Not Provided') AS field_of_study,
            ...
            WHERE l.Is_Active = 1  
            GROUP BY 
                p.id, p.firstname, p.lastname, p.email, p.contact_number, 
                p.Province, p.Seeking_Placement, l.Is_Active, 1.Is_Placed
            ORDER BY p.id DESC  
        """
        ...
        # ✅ Debug Data
        for row in rows:
            print(f"📌 Debugging Row Data: {row}")  # ✅ Print row data for debugging
        ...
        fetch_all_applicant_results_company = [
            {
                ...
                "email": row[2],
                "contact_number": row[3],
                "province": row[4],  # ✅ Use lowercase 'province'
                "seeking_placement": row[5],
                "is_active": "Active" if row[6] == 1 else "Inactive",  
                "is_placed": "Yes" if row[7] == 1 else "No"
            }
            for row in rows
        ]
